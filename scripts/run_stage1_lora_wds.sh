...
#   BATCH_SIZE=1 NUM_WORKERS=8 PREFETCH_FACTOR=2 NUM_EPOCHS=3 \
...
if [[ -z "$S1_CKPT" ]]; then
  echo "ERROR: Set S1_CKPT=/path/to/existing/stage1_avclip_checkpoint.pt" >&2
  exit 2
fi
if [[ ! -d "$REPO" ]]; then
...
if [[ ! -f "$LORA_SCRIPT" ]]; then
  echo "ERROR: LORA_SCRIPT does not exist: $LORA_SCRIPT" >&2
  echo "Put train_stage1_lora_wds.py at $REPO/scripts/train_stage1_lora_wds.py or set LORA_SCRIPT=/path/to/it" >&2
  exit 2
fi
if [[ ! -f "$CONFIG" ]]; then
...
echo "LORA_TARGET_MODE=$LORA_TARGET_MODE"
echo "LORA_RANK=$LORA_RANK"
echo "LORA_ALPHA=$LORA_ALPHA"
echo "MAX_CLIP_LEN_SEC=$MAX_CLIP_LEN_SEC"
